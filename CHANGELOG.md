...
### 兼容性
- 正式宿主验证保持 DSH 0.1.2-rc.1，预览宿主验证更新到 0.1.5-alpha.1。

### 交互
- 批注工具条优先出现在选区下方；编辑窗口支持拖动，高亮避开输入区域（#48、#49、#50）。

### 修复
- 输入区为空或只有空白时，发送纯批注，不再追加空的「提问：」或要求模型回答不存在的问题；纯批注仍可隐藏为标签，并支持刷新后解析及中英文切换（#52）。
